...
<div class="page-content">
  <nav class="page-breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item" aria-current="page">QC Rework Section</li>
    </ol>
  </nav>
...
<script>
  ...
  const productionSection = <?= json_encode($production) ?>;
  console.log(userRole,productionSection);
  let itemMap = {};
...
data.filter(item => item.not_good >= 0&& item.status_rework != undefined && item.curr_section !== 'WAREHOUSE' && (item.status_rework!=='done'||item.status_qc!=='done')) // added condition here
  .forEach(item => {
...
${
  !(userRole === 'administrator' || (userRole === 'supervisor' && productionSection === 'assembly'))
    ? 'disabled'
    : ''
}
...
.then(data => {
  ...
  // 🚫 Block due to insufficient stock
  if (insufficientItems.length > 0) {
    Swal.fire({
      icon: 'error',
      title: 'Cannot Proceed',
      html: `The following components don't have enough stock to be replaced:<br><ul style="text-align: left;">${
  ...
  // ⚠️ Alert on any warning/critical stock levels
...
function submitInspection() {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      openQRModal(item.material_no, window.pendingTimeOutData, 'timeOut',rework,replace);
    }
  });
}
...
function openQRModal(materialId, item, mode,rework,replace) {
  console.log(item)
